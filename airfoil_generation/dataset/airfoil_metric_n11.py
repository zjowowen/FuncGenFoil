...
class Fit_airfoil_11_Torch:
    ...
    def compute_parsec_features(self, airfoil: torch.Tensor) -> torch.Tensor:
        ...
        feats = torch.stack([
            ...
            xup,    yup,    yxxup,
            xlo,    ylo,    yxxlo,
            torch.tensor((yte_up + yte_lo)/2.0, device=self.device, dtype=self.dtype),
    ...
    def get_rle(self) -> torch.Tensor:
        ...
        K = torch.abs(dx1_t*dy2_t - dx2_t*dy1_t) / (dx1_t.pow(2) + dy1_t.pow(2)).pow(1.5)
        return 1.0 / K
    ...
    def get_up(self):
        ...
        return (torch.tensor(xup,   device=self.device, dtype=self.dtype),
    ...
    def get_lo(self):
        ...
        return (torch.tensor(xlo,   device=self.device, dtype=self.dtype),
...
if __name__ == "__main__":
    ...
    yt = 0.1 * (0.2969*np.sqrt(x) - 0.1260*x -0.3516*x**2 + 0.2843*x**3 -0.1015*x**4)
    xp = np.concatenate([x, x[::-1]])
    yp = np.concatenate([yt, -yt[::-1]])
    foil = torch.from_numpy(np.stack([xp,yp],axis=1)).float()

    fitter = Fit_airfoil_11_Torch(iLE=100,
                             inner_steps=2000,
                             lr=0.005,
