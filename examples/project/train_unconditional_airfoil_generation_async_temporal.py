...
from airfoil_generation.model.async_temporal_optimal_transport_functional_flow_model import (
    AsyncTemporalOptimalTransportFunctionalFlow,
    AsyncTemporalScheduler,
)
from airfoil_generation.training.optimizer import CosineAnnealingWarmupLR
...
def main(args):
    ...
    config = EasyDict(
        dict(
            ...
            flow_model=dict(
                ...
                temporal_gaussian_process=dict(
                    type=args.kernel_type,
                    args={
                        "matern": dict(
                            device=device,
                ...
                model=dict(
                    ...
                    args=dict(
                        backbone=dict(
                            type="AsyncTemporalFourierNeuralOperator",
                            args=dict(
                                modes=args.modes,
                                vis_channels=1,
                                hidden_channels=256,
    ...
    flow_model = AsyncTemporalOptimalTransportFunctionalFlow(
    ...
    for iteration in track(
        ...
        if iteration % config.parameter.eval_rate == 0:
            flow_model.eval()
            with torch.no_grad():
                async_temporal_scheduler = AsyncTemporalScheduler(
                    device=device,
                )
                async_temporal_scheduler.set_initial_time(
                    t=torch.zeros(size=[9, 1, 257], device=device)
                )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--model_path", "-p", default=None, type=str, help="Model load path."
    )
    argparser.add_argument(
    ...
    argparser.add_argument(
        "--project_name",
        type=str,
        default="airfoil-unconditional-training-async-temporal",
        help="Project name",
    )
    argparser.add_argument(
        "--iterations",
